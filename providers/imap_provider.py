...
class ImapProvider(ABC):
    ...
    def __init__(self, imap_server_url, auth):
    ...
    def load_filters(self, filter_file: str):
        try:
            with open(filter_file, 'r') as file:
                filter = yaml.safe_load(file)
                self.filters = FilterAction.create_filter_actions(filter)
                logger.info("FilterActions loaded: {}".format(filter))
...
class GmailIMAPProvider(ImapProvider):
    ...
    def __init__(self, credentials_file: str):
        super().__init__(self.GMAIL_IMAP_SERVER_URL, credentials_file)
